...
def get_top_k_recall(top_k: List[int], db: torch.Tensor, 
    ...
    # Assert that the number of queries matches the ground truth entries
    assert qu.shape[0] == len(gt_pos), \
        f"Number of queries ({qu.shape[0]}) does not match number of ground truth entries ({len(gt_pos)})"
...
class VLAD:
    ...
    def fit(self, train_descs: Union[np.ndarray, torch.Tensor, None]):
        ...
        if self.can_use_cache_vlad():
            ...
            if hasattr(self.kmeans, 'device'):
                train_descs = train_descs.to(self.kmeans.device)
            else:
                train_descs = train_descs.cpu()
            self.kmeans.fit(train_descs)
            if hasattr(self.kmeans, 'device'):
                self.c_centers = self.kmeans.centroids.to(self.kmeans.device)
            else:
                self.c_centers = self.kmeans.centroids.cpu()
            if self.cache_dir is not None:
    ...
    def generate(self, query_descs: Union[np.ndarray, torch.Tensor],
        ...
        un_vlad = torch.zeros(self.num_clusters * self.desc_dim, device=self.device)
        if self.vlad_mode == 'hard':
            ...
            if cache_id is not None and self.can_use_cache_vlad() \
                ...
                # Ensure centroids and input are on the same device
                qd = query_descs
                centroids = self.kmeans.centroids
                if qd.device != centroids.device:
                    centroids = centroids.to(qd.device)
                    self.kmeans.centroids = centroids
                labels = self.kmeans.predict(qd)   # [q]
                if cache_id is not None and self.can_use_cache_vlad():
                    torch.save(labels, 
                            f"{self.cache_dir}/{cache_id}_l.pt")
    ...
    def generate_res_vec(self, 
        ...
        # Ensure query_descs is on the same device as cluster centers
        ...
        if cache_id is not None and self.can_use_cache_vlad() and \
            ...
            residuals = torch.load(
                    f"{self.cache_dir}/{cache_id}_r.pt").to(self.device)
        else:
            if self.norm_descs:
