...
def calculate_distances_and_AP(query_embed, positive_embeds, negative_embeds):
    ...
    all_labels = torch.cat([torch.ones_like(pos_distances), torch.zeros_like(neg_distances)])
    ...
    precisions = torch.cumsum(sorted_labels, dim=0) / torch.arange(1, len(sorted_labels) + 1, device=sorted_labels.device)
    ...
    with torch.no_grad():
        for _ in range(num_evaluations):
            ...
            for i, (query, positive, negative) in enumerate(dataloader):
                ...
                ap = calculate_distances_and_AP(query_embed[0], positive_embed, negative_embed)
                batch_APs.append(ap)
...
def train():
    ...
    # Initialize dataset
    locations = [(lat, lon, config['data']['base_height']) 
    ...
    for epoch in range(config['training']['num_epochs']):
        ...
        print(f'Epoch {epoch+1} - Avg Loss: {avg_epoch_loss:.4f}, Mean AP: {mean_AP:.4f} ± {std_AP:.4f}')
